...
def format_prayer_times(times, date_obj=None):
    """
    Форматирует время намазов для красивого вывода (один день).
    
    Использует пробелы для точного выравнивания, как в примере:
    Фаджр:         06:25
    Восход:        07:45
    
    Аргументы:
        times (dict): Словарь с временами намазов
        date_obj (datetime, optional): Дата для вывода
    
    Возвращает:
        str: Отформатированная строка с расписанием
    """
    if not times:
    ...
    # Словарь с русскими названиями и эмодзи для каждого намаза
    detailed_names = {
        'Fajr': '🌄 Фаджр:',
        'Sunrise': 'Восход:',
        'Duhr': '☀️ Зухр:',
    ...
    # === ШАГ 1: Находим максимальную длину названия для выравнивания ===
    max_name_length = 0
    for prayer in DETAILED_PRAYER_ORDER:
        if prayer in detailed_names:
            name = detailed_names[prayer]
            # Убираем эмодзи и двоеточие для точного расчета длины
            clean_name = name.replace('🌄', '').replace('☀️', '').replace('🌤', '')\
                            .replace('🌅', '').replace('🌙', '').replace(':', '').strip()
            if len(clean_name) > max_name_length:
                max_name_length = len(clean_name)
    
    # === ШАГ 2: Выводим основные намазы (первые 6) ===
    main_prayers = ['Fajr', 'Sunrise', 'Duhr', 'Asr', 'Maghrib', 'Isha']
    for prayer in main_prayers:
        if prayer in times:
            name = detailed_names[prayer]
            time_str = times[prayer]
            
            # Убираем эмодзи и двоеточие для расчета длины
            clean_name = name.replace('🌄', '').replace('☀️', '').replace('🌤', '')\
                            .replace('🌅', '').replace('🌙', '').replace(':', '').strip()
            
            # Рассчитываем сколько пробелов нужно для выравнивания
            # +3 - дополнительный отступ после двоеточия
            spaces_needed = max_name_length - len(clean_name) + 3
            
            # Добавляем строку с выровненным временем
            text += f"{name}{' ' * spaces_needed}{time_str}\n"
    
    # Пустая строка для разделения групп
    text += "\n"
    
    # === ШАГ 3: Выводим ночные времена ===
    night_prayers = ['FirstThird', 'Midnight', 'LastThird']
    for prayer in night_prayers:
        if prayer in times:
            name = detailed_names[prayer]
            time_str = times[prayer]
            
            # Убираем двоеточие для расчета длины
            clean_name = name.replace(':', '').strip()
            
            # Рассчитываем пробелы для выравнивания
            spaces_needed = max_name_length - len(clean_name) + 3
